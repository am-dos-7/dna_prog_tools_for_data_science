...
def loss_l0(y_pred, y) -> float:
        return np.sum(y_pred != y)/y.shape[0]

def loss_l1(y_pred, y) -> float:
        return loss_l0(y_pred,y)/y.shape[0]
        # try defining another loss function here
...
def mode(y):
    ...
    if len(y)==0:
        ...
        return stats.mode(y.flatten(),axis=None, keepdims=True)[0][0]
